...
class SyncSubtitles2Videos:
    def __init__(self, all_videos, db_path):
        ...
        self.db_folders_path = sorted(
            self.db_folders_path, key=lambda x: int(x.split(" v")[-1]), reverse=True
        )
    ...
    def find_all_left_signaler(self):
        ...
        for k, f in tqdm(enumerate(self.folders), total=len(self.folders)):
            ...
            if (
                signaling_1_quiet_moments is not None
                and signaling_2_quiet_moments is not None
            ):
                if (
                    len(signaling_1_quiet_moments) == 0
                    and len(signaling_2_quiet_moments) == 0
                ):
                    continue
            ...
            if signaling_1_quiet_moments is None:
                ...
                signaling_1_quiet_moments = [
                    dict(beg=curr_beg_pos, end=curr_beg_pos + 5000)
                ]

            elif signaling_2_quiet_moments is None:
                curr_signaler_quiet = 2
                curr_beg_pos = self.all_videos[self.all_videos.folder == f].beg.min()
                signaling_2_quiet_moments = [
                    dict(beg=curr_beg_pos, end=curr_beg_pos + 5000)
                ]
            ...
            quiet_to_send = (
                signaling_1_quiet_moments
                if curr_signaler_quiet == 1
                else signaling_2_quiet_moments
            )
            res = self.__find_left_signaling_in_single_video(
                vid, quiet_to_send, debug_frame=False
            )
            if res["Left"] == res["Right"]:
                continue

            left_id, right_id = self.__from_left_signaling_result(
                res, curr_signaler_quiet
            )
            all_persons_df = all_persons_df.append(
                pd.DataFrame(
                    dict(
                        folder_name=[f],
                        left_person=[left_id],
    ...
    def process_all_folders_2_sync_with_person_2_video(self):
        if not os.path.exists("vid_sync.csv"):
            pd.DataFrame(dict(v_part=[-1], left_id=[-1], right_id=[-1])).to_csv(
                "vid_sync.csv", mode="w"
            )
        for db_folder in tqdm(self.db_folders_path):
            ...
            all_videos_in_folder = list(
                filter(lambda x: ".mp4" in x, os.listdir(db_folder))
            )
            vid_numbers = []
            ...
            all_videos_in_folder = [
                cv.VideoCapture(os.path.join(db_folder, x))
                for x in all_videos_in_folder
            ]

            bar_occurrences = self.db_path.replace("\\", "/").count("/") - 1
            folder_name = os.path.join(
                *(db_folder.replace("\\", "/").split("/")[bar_occurrences:])
            )
            first_sign_beg = self.all_videos[
                self.all_videos["folder"] == folder_name
            ].beg
            ...
            encodes, p_ids = self._find_faces_ids_and_embedings_sorted_left_2_right(
                all_videos_in_folder, show_video1_frame=True, v_part=v_part
            )
            ...
            pd.DataFrame(
                dict(v_part=[v_part], left_id=[p_ids[0]], right_id=[p_ids[1]])
    ...
    def __find_middle_xpoint_in_faces_bbox(frame):
        faces = face_recognition.face_locations(
            frame, number_of_times_to_upsample=1, model="cnn"
        )
        if len(faces) < 2:
            return None

        middle_xpoint = (np.abs(faces[0][3] - faces[1][3]) / 2) + min(
            [faces[0][3], faces[1][3]]
        )
    ...
    def _make_talkers_motion_check(vid, end_frame_time, x_middle, debug_frames=False):
        ...
        if x_middle is None:
            ...
            while x_middle is None:
                ...
                if not ret:
                    return [], x_middle
                x_middle = SyncSubtitles2Videos.__find_middle_xpoint_in_faces_bbox(
                    frame
                )
    ...
    def _face_rec_create_encodings(
        self,
        vc,
        amount_faces: int = 20,
        show=False,
        ret_amount_faces=False,
        pbar: tqdm = None,
    ):
        """
    ...
    def _draw_bbox_in_frame(self, frame, face, id=None, identity_score=None):
        ...
        if id is not None and identity_score is not None:
            ...
            x_text = int(
                np.min([x1, x2]) - (cv.getTextSize(text, self.font, 0.5, 2)[0][0] // 2)
            )
            cv.putText(frame, text, (x_text, y1 - 5), self.font, 0.5, (50, 205, 50), 2)

        return frame

    def _find_faces_ids_and_embedings_sorted_left_2_right(
        self, captures, show_video1_frame=False, v_part=None
    ):
        ret, frame = captures[0].read()
        if not ret:
            return [], []

        p1_encodings, amount1_faces = self._face_rec_create_encodings(
            vc=captures[1], ret_amount_faces=True
        )
        p2_encodings, amount2_faces = self._face_rec_create_encodings(
        ...
        def check_faces_in_main_video_frame(frame_):
            faces = face_recognition.face_locations(
                frame_[:, :, ::-1], number_of_times_to_upsample=1, model="cnn"
            )
            if len(faces) < 2:
                while len(faces) < 2:
        ...
        if left_id == right_id:
            try_more = 30 * 5
            pbar_2 = tqdm(
                desc=f"get bad decision in video with vpart -> {v_part}", total=try_more
            )
        ...
        if show_video1_frame:
            ...
            cv.putText(
                frame, text, (x_text, faces[0][2] - 5), self.font, 0.5, (50, 205, 50), 2
            )
            ...
            ret, left_frame = (
                captures[vid1_id].read()
                if p1_str == "left"
                else captures[vid2_id].read()
            )
            ret, right_frame = (
                captures[vid1_id].read()
                if p2_str == "left"
    ...
    def __find_left_signaling_in_single_video(self, vid, holes, debug_frame=False):
        ...
        for c_hole_pos in range(len(holes)):
            ...
            most_white, x_middle = self._make_talkers_motion_check(
                vid, end_frame_time, x_middle, debug_frame
            )
            if len(most_white) == 0:
                continue
    ...
    def __find_where_signaling_is_quiet(self, folder_name, talker_id, pbar=None):
        ...
        talker_1_times = [
            (x[1].beg, x[1].end)
            for x in talker_1_signs.iterrows()
            if x[1].talker_id == talker_id
        ]
        ...
        for time_it in range(1, len(times_talking)):
            try:
                ...
                holes.append(
                    dict(
                        beg=times_talking[time_it - 1]["end"],
...
if __name__ == "__main__":
    sync_vid_db = SyncSubtitles2Videos(
