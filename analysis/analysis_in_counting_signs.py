...
for i in tqdm(range(20)):
    ...
    chart = sns.barplot(x="name", y="count", data=df_10percent, palette="Blues_d")
...
last_quartil = np.array(
    [x["count"] for x in sorted_signs[: int(len(sorted_signs) * 0.25)]]
)
...
slow_down = 2
final_video = cv.VideoWriter(
    "all_cuted_signs_slow_down.mp4",
    fourcc,
    video_fps // slow_down,
...
for sample in tqdm(more_than_30_samples):
    ...
    rnd_size = int(0.25 * len(cur_sign_all_videos))
    rnd_size = rnd_size if rnd_size > 1 else 1
    five_percent_random = np.random.randint(len(cur_sign_all_videos), size=rnd_size)
...
sign_db.folder_name = sign_db.folder_name.map(
    lambda x: os.path.join(
        *(x.replace("\\", "/").split("/")[libras_corpus_index_string:])
    )
)

sign_db.folder = sign_db.folder.map(
    lambda x: os.path.join(
        *(x.replace("\\", "/").split("/")[libras_corpus_index_string:])
    )
)

sign_db.estate = sign_db.estate.map(
    lambda x: os.path.join(
