...
db_folders_path = sorted(
    db_folders_path, key=lambda x: int(x.split(" v")[-1]), reverse=True
)


# %%
all_videos_in_folder = [
    cv.VideoCapture(os.path.join(db_folders_path[2], x))
    for x in os.listdir(db_folders_path[2])
    if ".mp4" in x
]
...
# %%
def face_rec_create_encodings(
    vc, amount_faces: int = 20, show=False, ret_amount_faces=False, pbar: tqdm = None
):
    encondings = []
    ...
    while count < amount_faces:
        ...
        face_location = face_recognition.face_locations(
            frame[:, :, ::-1], number_of_times_to_upsample=1, model="cnn"
        )
        if len(face_location) > 1:
            continue

        curr_face_encoding = face_recognition.face_encodings(
            frame[:, :, ::-1], face_location
        )
...
def make_embedings_from_reference_video(reference_vc, show_frame=False):
    ...
    while len(reference_embedings) < 2:
        ...
        if show_frame:
            ...
            if key == 27:
                break
        faces_in_reference = face_recognition.face_locations(
            reference_frame[:, :, ::-1], number_of_times_to_upsample=1, model="cnn"
        )
        reference_embedings = face_recognition.face_encodings(
            reference_frame[:, :, ::-1], faces_in_reference
        )
...
def make_bounding_box_from_encodings(
    vc, encodings_list, amount_frames=20, reference_embedings=None
):
    x_train, y_train = gen_train_set_from_encodings(
        encodings_list[0], encodings_list[1]
    )
    ...
    for _ in range(amount_frames):
        ...
        for face in faces_in_frame:
            ...
            cv.putText(
                frame,
                str(f"person_{np.argmax(identity)} {identity_score}"),
                (x1 + 5, y1 - 5),
                font,
                0.5,
                (50, 205, 50),
...
for db_folder in tqdm(db_folders_path):
    ...
    all_videos_in_folder = sorted(
        list(filter(lambda x: ".mp4" in x, os.listdir(db_folder))),
        key=lambda x: int(x.split(".mp4")[0][-1]),
    )
    all_videos_in_folder = [
        cv.VideoCapture(os.path.join(db_folder, x)) for x in all_videos_in_folder
    ]
    ...
    p1_encodings, amount1_faces = face_rec_create_encodings(
        vc=all_videos_in_folder[1], ret_amount_faces=True
    )
    p2_encodings, amount2_faces = face_rec_create_encodings(
        vc=all_videos_in_folder[2], ret_amount_faces=True
    )
    all_encodings.append(
        dict(
            encodings=[p1_encodings, p2_encodings],
    ...
    if amount1_faces == 1 and amount2_faces == 1:
        v1_frame, ids1, lf1 = make_bounding_box_from_encodings(
            all_videos_in_folder[0], [p1_encodings, p2_encodings], 1
        )
        reference_embedings = make_embedings_from_reference_video(
            all_videos_in_folder[0]
        )
        v2_frame, ids2, lf2 = make_bounding_box_from_encodings(
            all_videos_in_folder[1],
            [p1_encodings, p2_encodings],
            reference_embedings=reference_embedings,
        )
        v3_frame, ids3, lf3 = make_bounding_box_from_encodings(
            all_videos_in_folder[2], [p1_encodings, p2_encodings]
        )
...
clf = VotingClassifier(
    estimators=[("svc", svc), ("rfc", rfc), ("knn", knn)], voting="hard"
)
clf = knn
...
for it in range(len(all_videos_in_folder)):
    ...
    while True:
        ...
        for face in faces:
            ...
            cv.putText(
                img,
                str(f"person_{curr_max + 1} {identity_score}"),
                (x1 + 5, y1 - 5),
                font,
                0.5,
                (50, 205, 50),
...
def find_faces_ids_and_embedings_sorted_left_2_right(
    captures, show_video1_frame=False, fig_name=None
):
    ...
    left_encodings = face_recognition.face_encodings(
        frame[:, :, ::-1], [faces[left_id]]
    )
    right_encodings = face_recognition.face_encodings(
        frame[:, :, ::-1], [faces[right_id]]
    )
    left_encodings = left_encodings[0]
    right_encodings = right_encodings[0]

    p1_encodings, amount1_faces = face_rec_create_encodings(
        vc=captures[1], ret_amount_faces=True
    )  # , pbar=tqdm())
    p2_encodings, amount2_faces = face_rec_create_encodings(
        vc=captures[2], ret_amount_faces=True
    )  # , pbar=tqdm())
    ...
    if show_video1_frame:
        ...
        vid1_id = (
            knn.predict_proba(left_encodings.reshape((1, -1)))
            if p1_str == "left"
            else knn.predict_proba(right_encodings.reshape((1, -1)))
        )
        cv.putText(
            frame,
            str(f"person_{vid1_id} {p1_str}"),
            (int(f1_mid) + 5, faces[0][2] - 5),
            font,
            0.5,
            (50, 205, 50),
            2,
        ...
        cv.putText(
            frame,
            str(f"person_{vid2_id} {p2_str}"),
            (int(f2_mid) + 5, faces[0][2] - 5),
            font,
            0.5,
            (50, 205, 50),
            2,
        ...
        cv.circle(
            frame,
            center=(int(x_middle), faces[0][2]),
            radius=5,
            color=(0, 0, 255),
            thickness=-1,
        )
        ret, left_frame = (
            captures[vid1_id].read() if p1_str == "left" else captures[vid2_id].read()
        )
        ret, right_frame = (
            captures[vid1_id].read() if p2_str == "left" else captures[vid2_id].read()
        )
    ...
    return (
        [
            p1_encodings if left_id == 0 else p2_encodings,
            p1_encodings if right_id == 0 else p2_encodings,
        ],
...
def draw_bbox_in_frame(frame, face, id, identity_score):
    ...
    cv.putText(
        frame,
        str(f"person_{id} {identity_score}"),
        (x1 + 5, y1 - 5),
        font,
        0.5,
        (50, 205, 50),
...
for db_folder in tqdm(db_folders_path[:2]):
    ...
    encodes, p_ids = find_faces_ids_and_embedings_sorted_left_2_right(
        all_videos_in_folder, show_video1_frame=True, fig_name=v_part
    )
...
x_train_, y_train_ = gen_train_set_from_encodings(
    left_person_encodes, right_person_encodes
)
clf.fit(x_train_, y_train_)

# %%
ids = check_person_in_single_videos(
    all_videos_in_folder[2], is_left=True, clf=clf, pbar=tqdm()
)
...
# %%
for db_folder in tqdm(db_folders_path):
    non_ended_in_cam = sorted(
        list(filter(lambda x: "1.mp4" in x, os.listdir(db_folder))),
        key=lambda x: int(x.split(".mp4")[0][-1]),
    )
    ...
    if len(ended_in_cam) > 0:
        shutil.copy(
            os.path.join(db_folder, ended_in_cam[0]),
            os.path.join(db_folder, non_ended_in_cam[0]),
        )
    elif len(new_video) > 0:
        shutil.copy(
            os.path.join(db_folder, new_video[0]),
            os.path.join(db_folder, non_ended_in_cam[0]),
        )
    # else:
