...
class DataframePerson2Sign:
    ...
    def __init__(self, db, method="both", video_window=None):
        ...
        if method not in ["grad", "dist", "both"]:
            raise ValueError(
                "method not valid {}, "
                "correct are {}".format(method, ["grad", "dist", "both"])
            )

        self.method = "grad dist" if method == "both" else method
        self.db = db
        self.video_window = video_window if video_window is not None else [-1, -1]
        self.__method_map = {
            "grad": self.__make_derivate_around_a_frame,
    ...
    def __make_distance_between_frames(sample, beg, mid, end):
        ...
        all_frames_in_window = all_frames_in_window[
            all_frames_in_window["frame"] >= beg
        ]
        ...
        for p_id in persons_id_in_video:
            person_joints = all_frames_in_window[all_frames_in_window["person"] == p_id]
    ...
    def process_single_sample(self, sample):
        ...
        window_beg = (
            self.video_window[0]
            if self.video_window[0] != -1
            else (end_frame_video - beg_frame_video) // 2
        )

        window_end = (
            self.video_window[1]
            if self.video_window[1] != -1
            else (end_frame_video - beg_frame_video) // 2
        )
        ...
        for it in range(beg_frame_video, end_frame_video - window_size + 1):
            mid_curr_window = (
                it + window_beg
                if it + window_beg <= end_frame_video
                else end_frame_video
            )

            beg_curr_window = (
                it - window_beg
                if it - window_beg >= beg_frame_video
                else beg_frame_video
            )

            end_curr_window = (
                mid_curr_window + window_end
            ...
            for method_name in res:
                talker_id = self.__method_map[method_name](
                    sample, beg_curr_window, mid_curr_window, end_curr_window
                )
                who_talking.update(
                    {
                        method_name: {
