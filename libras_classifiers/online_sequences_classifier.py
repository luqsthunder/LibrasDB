...
net.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Flatten(), input_shape=(max_len_seq, amount_joints_used, 2)))
net.add(tf.keras.layers.LSTM(units=10))
net.add(tf.keras.layers.LSTM(units=4, activation='softmax'))
net.compile(optimizer='Adam', loss='categorical_crosentropy',
            metrics=['accuracy'])
