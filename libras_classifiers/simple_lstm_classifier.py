...
def save_pdf_fig(
    fit_history: tf.keras.callbacks.History,
    base_folder: str,
    acc_name: str,
    loss_name: str,
):
    ...
    plt.plot(
        np.arange(len(fit_history["val_accuracy"])),
        fit_history["val_accuracy"],
        label="val_accuracy",
    )
    plt.scatter(
        np.arange(len(fit_history["val_accuracy"])),
    ...
    plt.plot(
        np.arange(len(fit_history["val_accuracy"])),
        fit_history["val_loss"],
        label="val_loss",
    )
    plt.scatter(np.arange(len(fit_history["val_accuracy"])), fit_history["val_loss"])
    plt.plot(
        np.arange(len(fit_history["val_accuracy"])), fit_history["loss"], label="loss"
    )
...
def train_angle_lstm(
    ...
    return make_base_classifier(
        net,
        pose_db=db_used,
        pose=pose,
        amount_lstm=amount_lstm,
        lstm_1_units=lstm_1_units,
        lstm_2_units=lstm_2_units,
        lstm_3_units=lstm_3_units,
        dropout=dropout,
...
def train_lstm_xy_pose(
    ...
    if dense_units is not None:
        net.add(
            tf.keras.layers.TimeDistributed(
                tf.keras.layers.Dense(units=dense_units, activation="tanh")
            )
        )
...
def train_lstm_xy_pose_cnn(
    ...
    net.add(
        tf.keras.layers.TimeDistributed(
            tf.keras.layers.Conv1D(
                ...
                kernel_size=amount_joints_used // 2,
                activation="relu",
    ...
    return make_base_classifier(
        ...
        cnn_1_filters=cnn_1_filters,
        cnn_2_filters=cnn_2_filters,
    )
...
def make_base_classifier(
    ...
    lstms = create_lstms(
        [lstm_units[it] for it in range(amount_lstm)],
        dropout=dropout,
        dropout_recurrent=dropout_recurrent,
        input_shape=input_shape,
    )
    ...
    fit_res = net.fit(
        ...
        validation_data=pose_db.validation(),
    )
    # net.save('model.h5')

    cnn_str_params = (
        f"CNN Kernel {cnn_kernel} Filters {cnn_1_filters} {cnn_2_filters}"
        if cnn_kernel is not None
        and cnn_1_filters is not None
        and cnn_2_filters is not None
        else ""
    )

    acc_name = (
        f"Accuracy {pose} lstm-{amount_lstm} lstm-{lstm_1_units} lstm-{lstm_2_units} lstm {lstm_3_units} "
        f"drop {dropout} rdrop {dropout_recurrent} dense {dense_units}" + cnn_str_params
    )

    loss_name = (
        f"Loss {pose} lstm-{amount_lstm} lstm-{lstm_1_units} lstm-{lstm_2_units} lstm {lstm_3_units} "
        f"drop {dropout} rdrop {dropout_recurrent} dense {dense_units}" + cnn_str_params
    )
    ...
    return (
        base_folder + acc_name + ".pdf",
        base_folder + loss_name + ".pdf",
        fit_res.history,
    )
...
all_params_product = [
    {key: x[it] for it, key in enumerate(keys)} for x in list(itertools.product(*vals))
]
...
df_exp = (
    pd.read_csv(exp_path + csv_angle_name)
    if os.path.exists(exp_path + csv_angle_name)
    else pd.DataFrame()
)
...
for param in tqdm(all_params_product):
    ...
    acc_pdf_file, loss_pdf_file, history = train_angle_lstm(
        "angle-dense-no-derivate", use_derivative=False, **param
    )
...
for param in tqdm(all_params_product):
    ...
    tf.keras.backend.clear_session()
    acc_pdf_file, loss_pdf_file, history = train_angle_lstm(
        "angle-no-dense", dense_units=None, use_derivative=True, **param
    )
...
for param in tqdm(all_params_product):
    ...
    tf.keras.backend.clear_session()
    acc_pdf_file, loss_pdf_file, history = train_angle_lstm(
        "angle-no-dense-no-derivative", use_derivative=False, **param
    )
...
df_exp = (
    pd.read_csv(exp_path + csv_xy_no_dense_name)
    if os.path.exists(exp_path + csv_xy_no_dense_name)
    else pd.DataFrame()
)
...
for param in tqdm(all_params_product):
    ...
    tf.keras.backend.clear_session()
    try:
        acc_pdf_file, loss_pdf_file, history = train_lstm_xy_pose(
            "xy-pose", dense_units=None, **param
        )
    except:
...
df_exp = (
    pd.read_csv(exp_path + csv_xy_cnn_name)
    if os.path.exists(exp_path + csv_xy_cnn_name)
    else pd.DataFrame()
)
...
pose_angle = (
    "all_experiments_pose_angle_batch1.csv"  # pose em angulo + derivadas + densa
)
csv_angle_name_no_dense = "all_experiments_pose_angle_no_dense_derivatives_batch1.csv"  # pose em angulo + derivadas - densa
csv_angle_name_no_dense_no_dt = (
    "all_experiments_pose_angle_no_dense_no_derivatives_batch1.csv"
)
...
pose_angle_dt_no_dense_df = pd.read_csv(
    exp_path + csv_angle_name_no_dense
)  # pose em angulo + derivadas - densa
pose_angle_no_dt_no_dense_df = pd.read_csv(exp_path + csv_angle_name_no_dense_no_dt)

pose_angle_no_dt_dense_df = pose_angle_no_dt_no_dense_df[
    pose_angle_no_dt_no_dense_df.dense_units.isna()
].reset_index()
...
def make_subplot(df, name, desc=None):
    ...
    network = list(map(str, df.exp_name.tolist())) + list(
        map(str, df.exp_name.tolist())
    )

    dropout = list(map(float, df.dropout.tolist())) + list(
        map(float, df.dropout.tolist())
    )

    recurrent_dropout = list(map(float, df.dropout_recurrent.tolist())) + list(
        map(float, df.dropout_recurrent.tolist())
    )
...
all_df = all_df.append(
    make_subplot(
        pose_angle_dt_no_dense_df,
        "pose_angle_no_dt_no_dense",
        desc="OJA + $b_1$"
...
all_df = all_df.append(
    make_subplot(
        pose_angle_no_dt_no_dense_df,
        "pose_angle_no_dt_dense",
        desc="OJATD + $b_2$"
...
for exp_name in all_exp_names:
    exp = all_df[all_df["exp_names"] == exp_name].sort_values(
        by="all_acc", ascending=False
    )
    exp_acc = exp[exp["acc_kind"] == "acc"]
    curr_acc = exp_acc.iloc[0:amount_best_acc]
    for idx, row in curr_acc.iterrows():
        curr_val_acc = exp[
            (exp["acc_kind"] == "val_acc") & (exp["network"] == row.network)
        ]
...
experiment_amount_single = all_df[all_df["exp_names"] == "pose_angle"].exp_names.shape[
    0
]
...
for exp_name in experiments_name:
    only_this_experiment_df = sorted_experiments[
        sorted_experiments["exp_names"] == exp_name
    ].reset_index()
    exp_df_data = only_this_experiment_df.join(
        pd.DataFrame(dict(move=np.arange(only_this_experiment_df.shape[0])))
    )
    all_exp_sorted_with_move.append(exp_df_data)

all_exp_sorted_with_move = all_exp_sorted_with_move[0].append(
    all_exp_sorted_with_move[1:]
)
chart = sns.lineplot(
    data=all_exp_sorted_with_move, x="move", y="all_acc", hue="exp_names"
)
