...
class OcvVideoPlayer:
    ...
    def _get_opts_from_curr_vid(self):
        ...
        persons_id_sub = self.all_persons_from_subtitle[
            self.all_persons_from_subtitle["v_part"] == v_part
        ]
    ...
    def save_single_vid(self, pos):
        ...
        seen_vid_path = (
            f'../vid-folder/{curr_opts["sign"]}/seen/' + curr_opts["name"] + ".mp4"
        )
        if os.path.exists(seen_vid_path):
            return True

        vid_save = cv.VideoWriter(
            vid_path, fourcc, video_fps, (video_width, video_height)
        )
...
if __name__ == "__main__":
    ocv_video_player = OcvVideoPlayer(
        libras_corpus_path="d:/gdrive",
        sign_db_path="../sign_db_front_view",
