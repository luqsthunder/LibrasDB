...
db_angle = DBLoader2NPY(
    db_path="../Libras-db-folders", batch_size=1, angle_pose=True, no_hands=False
)
db_xy = DBLoader2NPY(
    db_path="../Libras-db-folders", batch_size=1, angle_pose=False, no_hands=False
)

db_angle_x, db_angle_y = db_angle.batch_load_samples(
    list(range(0, db_angle.db_length())), as_npy=False
)
db_xy_x, db_xy_y = db_xy.batch_load_samples(
    list(range(0, db_xy.db_length())), as_npy=False
)
angles_keys = [
...
for angle_sample, xy_sample in tqdm(zip(db_angle_x, db_xy_x), total=len(db_xy_x)):
    if xy_sample["Neck"].iloc[0][0] > 200.0:
        curr_angle_df = make_angle_df_from_xy(
            xy_sample, no_hands=True, body_angles=body_angle_pairs_to_7
        )
        curr_angle_df = curr_angle_df.fillna(0.0)
        res = np.isclose(
            curr_angle_df[angles_keys].values, angle_sample[angles_keys].values
        ).all()
        if not res:
            print(
                f" {curr_angle_df[angles_keys].values}, \n {angle_sample[angles_keys].values}"
            )
...
count = 0
for angle_sample, xy_sample, sample_name in tqdm(
    zip(db_angle_x, db_xy_x, db_angle.samples_path), total=len(db_xy_x)
):
    if xy_sample["Neck"].iloc[0][0] > 200.0:
...
for angle_sample, xy_sample, sample_y, it, sample_name in tqdm(
    iterator_samples, total=len(db_xy_x)
):
    if xy_sample["Neck"].iloc[0][0] > 200.0:
        sample_id_aux = []
...
np.random.seed(5)
samples_ids = (
    np.random.randint(0, 120, size=10).tolist()
    + np.random.randint(120, 240, size=10).tolist()
)
...
for sign in tqdm(signs_name):
    count_signs.append(
        dict(name=sign, count=sign_db[sign_db.sign == sign].sign.count())
    )
