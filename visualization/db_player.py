...
from PyQt5.QtWidgets import (QWidget, QLabel, QApplication, QPushButton, QHBoxLayout, QVBoxLayout, QSlider, QListWidget,
                             QListWidgetItem)
...
class App(QWidget):
    ...
    def init_ui(self):
        ...
        self.th = self.th_cls(**self.th_cls_kwargs, th_list=self.sample_list, th_slider=self.vid_time_slider)
...
class ViewDBCutVideos(OCVVideoThread):
    ...
    def make_matplotlib_2_npy(self, frame_pos, step_y=0.2, save_fig=False):
        if frame_pos is not None and str(frame_pos) in self.memory_angle_plot and not self.save_fig:
        ...
        ax: plt.axes = fig.gca()
        ax.set_ylim(-4, 4)
        ax.set_xlim(self.curr_sample_angles.frame.iloc[0], self.curr_sample_angles.frame.iloc[-1])
    ...
    def __render_all_matplotlib_frames(self):
        self.memory_angle_plot = {str(x[1].frame): self.make_matplotlib_2_npy(x[1].frame)
