...
class CheckBoxes(tkinter.Frame):
    ...
    def __init_ui(self):
        ...
        self.check_box_list = [
            tkinter.Checkbutton(
                self,
                text="Legenda 1: Pessoa: 1",
                variable=self.check_box_var_list[0],
                command=self.fn_ck,
            ).grid(row=0, column=0),
            tkinter.Checkbutton(
                self,
                text="Legenda 2: Pessoa: 1",
                variable=self.check_box_var_list[1],
                command=self.fn_ck,
            ).grid(row=0, column=1),
            tkinter.Checkbutton(
                self,
                text="Legenda 1: Pessoa: 2",
                variable=self.check_box_var_list[2],
                command=self.fn_ck,
            ).grid(row=1, column=0),
            tkinter.Checkbutton(
                self,
                text="Legenda 2: Pessoa: 2",
                variable=self.check_box_var_list[3],
                command=self.fn_ck,
            ).grid(row=1, column=1),
            tkinter.Checkbutton(
                self,
                text="Pessoa 1: video: 1",
                variable=self.check_box_var_list[4],
                command=self.fn_ck,
            ).grid(row=2, column=0),
            tkinter.Checkbutton(
                self,
                text="Pessoa 2: video: 1",
                variable=self.check_box_var_list[5],
                command=self.fn_ck,
            ).grid(row=2, column=1),
            tkinter.Checkbutton(
                self,
                text="Pessoa 1: video: 2",
                variable=self.check_box_var_list[6],
                command=self.fn_ck,
            ).grid(row=3, column=0),
            tkinter.Checkbutton(
                self,
                text="Pessoa 2: video: 2",
                variable=self.check_box_var_list[7],
                command=self.fn_ck,
            ).grid(row=3, column=1),
        ]
...
class MakeDB3Videos:
    def __init__(self, vid_db_path, vid_df, all_video_df, window, window_title):
        ...
        self.all_video_df = (
            all_video_df
            if isinstance(all_video_df, pd.DataFrame)
            else pd.read_csv(all_video_df)
        )
        ...
        self.next_folder_btn = tkinter.Button(
            window, text="Next", width=50, command=self.next_folder_btn_fn
        )
    ...
    def update_caps(self):
        ...
        curr_folder_name = (
            self.all_video_df.folder_name.iloc[self.cur_vid_pos]
            .replace("\\", "/")
            .split("/")[:-1]
        )
        curr_folder_name = os.path.join(*curr_folder_name)
        curr_folder_path = os.path.join(self.vid_db_path, curr_folder_name)
        videos_path_at_curr_folder = list(
            filter(lambda x: ".mp4" in x, os.listdir(curr_folder_path))
        )
        videos_path_at_curr_folder = list(
            map(lambda x: os.path.join(self.vid_db_path, x), videos_path_at_curr_folder)
        )
        ...
        self.cur_beg = (
            self.all_video_df[self.all_video_df.folder_name == curr_folder_name_in_df]
            .iloc[0]
            .beg
            // 30
        )
    ...
    def run(self):
        ...
        frames = list(
            filter(lambda x: x is not None, [x.read()[1] for x in self.cv_caps])
        )
        if len(frames) > 0:
