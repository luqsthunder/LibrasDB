...
from libras_classifiers.generate_dataframe_person_2_sign import DataframePerson2Sign
...
class PoseCentroidTracker:
    ...
    def __init__(
        ...
        self.pose_extractor = (
            OpenposeExtractor(openpose_path=openpose_path)
            if isinstance(openpose_path, str)
            else openpose_path
        )
        self.person_2_sign = DataframePerson2Sign(None)

        self.centroids_df = (
            pd.read_csv(centroids_df_path) if centroids_df_path is not None else None
        )
        self.folder_2_track = folder_2_track
        if self.centroids_df is not None:
            self.centroids_df["centroid"] = self.centroids_df["centroid"].apply(
                self.parse_npy_vec_str
            )
    ...
    def retrive_persons_centroid_from_sign_df(self, folder_path, db_path, pbar=None):
        ...
        pandas.DataFrame
            ...
            pessoa.
        """
        persons_alone = self.signaler_find.find_where_signalers_talks_alone(folder_path)

        src_file_name = os.path.join(db_path, folder_path)
        video = cv.VideoCapture(src_file_name)

        df_persons_centroid_video = pd.DataFrame(
            columns=["folder", "talker_id", "centroid"]
        )
        ...
        for person_sub_id, alone_talk in persons_alone.items():
            ...
            succ = video.set(
                cv.CAP_PROP_POS_FRAMES, float(alone_talk["beg"] / frame_time)
            )
            if not succ:
                raise RuntimeError(
                    f"Could not set video position. The video is: {folder_path}"
                )
            ...
            while video.get(cv.CAP_PROP_POS_FRAMES) <= end_pos:
                ...
                if not ret:
                    raise RuntimeError(
                        f"Video Ended Beforme was expected."
                ...
                curr_centroids = list(map(self.make_xy_centroid, dt.poseKeypoints))
                ...
                persons_centroids[0].append(curr_centroids[left_id])
                persons_centroids[1].append(curr_centroids[right_id])
            ...
            talking_person_id = self.person_2_sign.process_single_sample(pose_df)
            ...
            df_persons_centroid_video = df_persons_centroid_video.append(
                curr_data, ignore_index=True
            )
    ...
    def subdivide_2_persons_scene(self, folder):
        if self.centroids_df is None:
            raise ValueError(
                "self.centroids_df must non None if wanted to " "divide scene"
            )
    ...
    def filter_persons_by_x_mid(self, dt, first_x_mid):
        ...
        if dt.poseKeypoints.size > 1:
            ...
            if len(curr_body_centroids) > 1:
                left_person_in_dt = 0 if curr_body_centroids[0][0] < x_mid_body else 1
                right_person_in_dt = 1 if left_person_in_dt == 0 else 0
            else:
                left_person_in_dt = (
                    0 if curr_body_centroids[0][0] < first_x_mid else None
                )
                right_person_in_dt = (
                    0 if curr_body_centroids[0][0] > first_x_mid else None
                )
        ...
        if dt.handKeypoints[0].size > 1:
            curr_r_hands_centroids = list(map(self.make_xy_centroid, dt.poseKeypoints))
            ...
            if len(curr_r_hands_centroids) > 1:
                left_person_r_hand_dt = (
                    0 if curr_r_hands_centroids[0][0] < x_mid_hands else 1
                )
                right_person_r_hand_dt = 1 if left_person_r_hand_dt == 0 else 0
            else:
                left_person_r_hand_dt = (
                    0 if curr_r_hands_centroids[0][0] < first_x_mid else None
                )
                right_person_r_hand_dt = (
                    0 if curr_r_hands_centroids[0][0] < first_x_mid else None
                )
        else:
        ...
        return (
            [left_person_in_dt, left_person_r_hand_dt],
