...
class ConvertPoseToAngle:
    def __init__(
        self,
        db_pose_path,
        base_joint_xaxis=None,
        base_joint_yaxis=None,
        joint_list=None,
    ):
        """
        ...
        if joint_list is None:
            ...
            self.edges_list = list(map(lambda x: (x[0] + 2, x[1] + 2), self.edges_list))
            ...
            left_hand_edges = [
                (x[0] + left_hand_position, x[1] + left_hand_position)
                for x in left_hand_edges
            ]

            right_hand_edges = copy(hand_edges)
            right_hand_edges = [
                (x[0] + right_hand_position, x[1] + right_hand_position)
            ...
            head_edges = [
                (x[0] + head_position, x[1] + head_position) for x in head_edges
            ]
            ...
            self.edges_list = list(
                map(lambda x: (df_cols[x[0]], df_cols[x[1]]), self.edges_list)
            )
                ...
                for it in range(len(single_video.index)):
                    ...
                    for it2, edge in enumerate(self.edges_list):
                        try:
                            ...
                            if first_point[2] > 0.20 and second_point[2] > 0.20:
                                ...
                                cv.line(black_img, first_point, second_point, color)
    ...
    def convert_single_pose(self, curr_pose):
        ...
        for key in curr_pose.keys():
            ...
            angle = np.arccos(
                (np.linalg.norm(first_joint) * np.linalg.norm(second_joint))
                / np.dot(first_joint, second_joint)
            )
            angle_sign = np.sign(
                first_joint[0] * second_joint[1] - first_joint[1] * second_joint[0]
            )
    ...
    def make_base_axis(self, curr_pose):
        ...
        """
        x_axis = list(
            map(lambda x: np.array(x), curr_pose[self.base_joint_xaxis].values)
        )
        y_axis = list(
            map(lambda x: np.array(x), curr_pose[self.base_joint_yaxis].values)
        )

        regressor = LinearRegression()

        regressor.fit(
            np.array([[x[0]] for x in x_axis]), np.array([[x[1]] for x in x_axis])
        )
        x_axis_line = [regressor.coef_, regressor.intercept_]

        regressor.fit(
            np.array([y[0] for y in y_axis]), np.array([y[1] for y in y_axis])
        )
